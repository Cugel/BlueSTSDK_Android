...
class NodeService(
    ...
    fun getConfigControlUpdates(): Flow<FeatureResponse> {
        return configControlService.configControlUpdates.transform { rawData ->
            ...
            characteristicWithFeatures.flatMap { it.features }
    ...
    fun getFeatureUpdates(features: List<Feature<*>>): Flow<FeatureUpdate<*>> {
    ...
    suspend fun writeFeatureCommand(
        ...
        if (isExtendedFeatureCommand.not()) { // if is not an extended feature no response will be generated from connected board
