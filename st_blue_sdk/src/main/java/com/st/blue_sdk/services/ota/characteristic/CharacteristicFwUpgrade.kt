...
class CharacteristicFwUpgrade(
        ...
        supervisorScope {
            ...
            while (uploadedData < fileSize) {
                ...
                val writtenDataCount = transferFwFile(
                    ...
                    fileSize,
                    uploadedData,
                    otaNodeService.getNode().maxPayloadSize
                )
    ...
    private suspend fun startFwUpload() {
        ...
        val startUploadRequest = StartUpload(otaControlFeature, commandId, updateArgs.address)
    ...
    private suspend fun isOTAMode(node: Node): Boolean {
        ...
        if (node.advertiseInfo.getDeviceId() == OTA_NODE_ID) {
            return true
        }
