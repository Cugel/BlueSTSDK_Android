...
import com.st.blue_sdk.features.exported.AudioOpusConfFeature as ExportedAudioOpusConfFeature
...
class AudioServiceImpl @Inject constructor(
    ...
    private fun sendAudioStream(featureName: String, nodeId: String, data: ShortArray): Boolean {
        ...
        val audioFeature = nodeService.getNodeFeatures().first {
            requiredOpusFeatures.second == it.name || requiredADPCMFeatures.second == it.name
        }
    ...
    override suspend fun sendVoiceAudioStream(nodeId: String, data: ShortArray) =
        sendAudioStream(AudioOpusVoiceFeature.NAME, nodeId, data)

    override suspend fun sendMusicAudioStream(nodeId: String, data: ShortArray) =
        sendAudioStream(AudioOpusMusicFeature.NAME, nodeId, data)
